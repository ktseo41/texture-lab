...
- **프랙탈 글래스** — 무늬 유리 효과: 굴절, 색 분산, 서리, 라인 페이드. 실제 유리 사진을 기준으로 맞춘 유리 무늬 8종 — 세로 줄(기존 / 입체), 가로 줄, 물결 줄, 피라미드 격자, 그리고 칸마다 뒤 그림이 작게 반복되는 네모·마름모·벌집 렌즈
...
- **프리셋** — 팩토리 6종 + 내 프리셋 저장 (localStorage / JSON 내보내기)
- **PNG 내보내기** (임의 캔버스 크기), 한/영/일 UI
- **모바일 레이아웃** — 좁은 화면에서 스테이지와 패널이 세로로 쌓이고, 미리보기를 탭하면 전체 보기/실제 크기 전환 (크기·배율 표시)
...
src/
  engine/          렌더 엔진 (UI와 완전 분리)
    ...
    glass.js       프랙탈 글래스: 무늬 8종(줄, 피라미드, 렌즈 칸), 굴절, 색 분산, 서리, 라인 페이드
    grain.js       필름 그레인(노이즈 캐시) + 먼지/잉크 플레이크
    pipeline.js    source → halftone → fractal glass → speckle → grain 오케스트레이션,
                   스테이지별 캐싱
  state/
