...
- **프랙탈 글래스** — 무늬 유리 효과: 굴절, 색 분산, 서리, 라인 페이드. 실제 유리 사진을 기준으로 맞춘 유리 무늬 4종 — 세로 줄, 물결 줄, 피라미드 격자, 그리고 굴절을 올릴수록 또렷함 → 모자이크 → 칸마다 반복으로 바뀌는 네모 렌즈
...
src/
  engine/          렌더 엔진 (UI와 완전 분리)
    ...
    glass.js       프랙탈 글래스: 무늬 4종(줄, 피라미드, 렌즈 칸), 굴절, 색 분산, 서리, 라인 페이드
